...
interface IPrivilegedService {
    ...
    boolean writeFile(String content, String path);
    ...
    boolean saveBitmapFromFile(String sourcePath, String destPath);

    // Runs command[] and returns true if the process exits with code 0.
    boolean execute(in String[] command);
    ...
    String executeWithOutput(int maxLines, long timeoutMs, boolean returnError, in String[] command);

    // Enables an accessibility service by appending componentName
    // ("package/ClassName") to the secure setting.
    boolean enableAccessibilityService(String componentName);
